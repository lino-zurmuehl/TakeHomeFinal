...
twitter_names <- read_csv("https://raw.githubusercontent.com/oduwsdl/US-Congress/master/116thCongress/116Congress.csv") %>% 
  select(RealName = `Wikipedia  Names`, twitter_name = CSPAN)

data_name <- data %>%
  rename (twitter_name = screen_name) %>%
  left_join(data_name, twitter_names, by = "twitter_name")
...
data <- data %>%
  left_join(wiki_senators, by = "name")

data <- data %>%
  left_join(wiki_representatives, by = "name")
